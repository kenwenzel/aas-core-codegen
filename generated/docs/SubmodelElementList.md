

# Class: SubmodelElementList 


_A submodel element list is an ordered list of submodel elements._





URI: [aas:SubmodelElementList](https://admin-shell.io/aas/3/0/RC02/SubmodelElementList)





```mermaid
 classDiagram
    class SubmodelElementList
    click SubmodelElementList href "../SubmodelElementList/"
      SubmodelElement <|-- SubmodelElementList
        click SubmodelElement href "../SubmodelElement/"
      
      SubmodelElementList : category
        
      SubmodelElementList : checksum
        
      SubmodelElementList : description
        
      SubmodelElementList : displayName
        
      SubmodelElementList : embeddedDataSpecifications
        
          
    
        
        
        SubmodelElementList --> "*" EmbeddedDataSpecification : embeddedDataSpecifications
        click EmbeddedDataSpecification href "../EmbeddedDataSpecification/"
    

        
      SubmodelElementList : extensions
        
          
    
        
        
        SubmodelElementList --> "*" Extension : extensions
        click Extension href "../Extension/"
    

        
      SubmodelElementList : idShort
        
      SubmodelElementList : kind
        
          
    
        
        
        SubmodelElementList --> "0..1" ModelingKind : kind
        click ModelingKind href "../ModelingKind/"
    

        
      SubmodelElementList : orderRelevant
        
      SubmodelElementList : qualifiers
        
          
    
        
        
        SubmodelElementList --> "*" Qualifier : qualifiers
        click Qualifier href "../Qualifier/"
    

        
      SubmodelElementList : semanticId
        
          
    
        
        
        SubmodelElementList --> "0..1" Reference : semanticId
        click Reference href "../Reference/"
    

        
      SubmodelElementList : semanticIdListElement
        
          
    
        
        
        SubmodelElementList --> "0..1" Reference : semanticIdListElement
        click Reference href "../Reference/"
    

        
      SubmodelElementList : supplementalSemanticIds
        
          
    
        
        
        SubmodelElementList --> "*" Reference : supplementalSemanticIds
        click Reference href "../Reference/"
    

        
      SubmodelElementList : typeValueListElement
        
          
    
        
        
        SubmodelElementList --> "1" AasSubmodelElements : typeValueListElement
        click AasSubmodelElements href "../AasSubmodelElements/"
    

        
      SubmodelElementList : value
        
          
    
        
        
        SubmodelElementList --> "*" SubmodelElement : value
        click SubmodelElement href "../SubmodelElement/"
    

        
      SubmodelElementList : valueTypeListElement
        
          
    
        
        
        SubmodelElementList --> "0..1" DataTypeDefXsd : valueTypeListElement
        click DataTypeDefXsd href "../DataTypeDefXsd/"
    

        
      
```





## Inheritance
* [SubmodelElement](SubmodelElement.md) [ [Referable](Referable.md) [HasKind](HasKind.md) [HasSemantics](HasSemantics.md) [Qualifiable](Qualifiable.md) [HasDataSpecification](HasDataSpecification.md)]
    * **SubmodelElementList**



## Slots

| Name | Cardinality and Range | Description | Inheritance |
| ---  | --- | --- | --- |
| [orderRelevant](orderRelevant.md) | 0..1 <br/> [Boolean](Boolean.md) | Defines whether order in list is relevant | direct |
| [semanticIdListElement](semanticIdListElement.md) | 0..1 <br/> [Reference](Reference.md) | Semantic ID the submodel elements contained in the list match to | direct |
| [typeValueListElement](typeValueListElement.md) | 1 <br/> [AasSubmodelElements](AasSubmodelElements.md) | The submodel element type of the submodel elements contained in the list | direct |
| [value](value.md) | * <br/> [SubmodelElement](SubmodelElement.md) | Submodel element contained in the list | direct |
| [valueTypeListElement](valueTypeListElement.md) | 0..1 <br/> [DataTypeDefXsd](DataTypeDefXsd.md) | The value type of the submodel element contained in the list | direct |
| [category](category.md) | 0..1 <br/> [String](String.md) | The category is a value that gives further meta information w | [Referable](Referable.md) |
| [checksum](checksum.md) | 0..1 <br/> [String](String.md) | Checksum to be used to determine if an Referable (including its aggregated ch... | [Referable](Referable.md) |
| [description](description.md) | * <br/> [LangString](LangString.md) | Description or comments on the element | [Referable](Referable.md) |
| [displayName](displayName.md) | * <br/> [LangString](LangString.md) | Display name | [Referable](Referable.md) |
| [idShort](idShort.md) | 0..1 <br/> [String](String.md) | In case of identifiables this attribute is a short name of the element | [Referable](Referable.md) |
| [kind](kind.md) | 0..1 <br/> [ModelingKind](ModelingKind.md) | Kind of the element: either type or instance | [HasKind](HasKind.md) |
| [semanticId](semanticId.md) | 0..1 <br/> [Reference](Reference.md) | Identifier of the semantic definition of the element | [HasSemantics](HasSemantics.md) |
| [supplementalSemanticIds](supplementalSemanticIds.md) | * <br/> [Reference](Reference.md) | Identifier of a supplemental semantic definition of the element | [HasSemantics](HasSemantics.md) |
| [qualifiers](qualifiers.md) | * <br/> [Qualifier](Qualifier.md) | Additional qualification of a qualifiable element | [Qualifiable](Qualifiable.md) |
| [embeddedDataSpecifications](embeddedDataSpecifications.md) | * <br/> [EmbeddedDataSpecification](EmbeddedDataSpecification.md) | Embedded data specification | [HasDataSpecification](HasDataSpecification.md) |
| [extensions](extensions.md) | * <br/> [Extension](Extension.md) | An extension of the element | [HasExtensions](HasExtensions.md) |










## Identifier and Mapping Information






### Schema Source


* from schema: https://admin-shell.io/aas/3/0/RC02




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | aas:SubmodelElementList |
| native | aas:SubmodelElementList |






## LinkML Source

<!-- TODO: investigate https://stackoverflow.com/questions/37606292/how-to-create-tabbed-code-blocks-in-mkdocs-or-sphinx -->

### Direct

<details>
```yaml
name: SubmodelElementList
description: A submodel element list is an ordered list of submodel elements.
from_schema: https://admin-shell.io/aas/3/0/RC02
is_a: SubmodelElement
attributes:
  orderRelevant:
    name: orderRelevant
    description: Defines whether order in list is relevant. If 'orderRelevant' = False
      then the list is representing a set or a bag.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    domain_of:
    - SubmodelElementList
    range: boolean
  semanticIdListElement:
    name: semanticIdListElement
    description: Semantic ID the submodel elements contained in the list match to.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    domain_of:
    - SubmodelElementList
    range: Reference
  typeValueListElement:
    name: typeValueListElement
    description: The submodel element type of the submodel elements contained in the
      list.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    domain_of:
    - SubmodelElementList
    range: AasSubmodelElements
    required: true
  value:
    name: value
    description: Submodel element contained in the list.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    domain_of:
    - Blob
    - DataSpecificationIec61360
    - Extension
    - File
    - Key
    - MultiLanguageProperty
    - OperationVariable
    - Property
    - Qualifier
    - ReferenceElement
    - SpecificAssetId
    - SubmodelElementCollection
    - SubmodelElementList
    - ValueReferencePair
    range: SubmodelElement
    multivalued: true
  valueTypeListElement:
    name: valueTypeListElement
    description: The value type of the submodel element contained in the list.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    domain_of:
    - SubmodelElementList
    range: DataTypeDefXsd

```
</details>

### Induced

<details>
```yaml
name: SubmodelElementList
description: A submodel element list is an ordered list of submodel elements.
from_schema: https://admin-shell.io/aas/3/0/RC02
is_a: SubmodelElement
attributes:
  orderRelevant:
    name: orderRelevant
    description: Defines whether order in list is relevant. If 'orderRelevant' = False
      then the list is representing a set or a bag.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: orderRelevant
    owner: SubmodelElementList
    domain_of:
    - SubmodelElementList
    range: boolean
  semanticIdListElement:
    name: semanticIdListElement
    description: Semantic ID the submodel elements contained in the list match to.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: semanticIdListElement
    owner: SubmodelElementList
    domain_of:
    - SubmodelElementList
    range: Reference
  typeValueListElement:
    name: typeValueListElement
    description: The submodel element type of the submodel elements contained in the
      list.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: typeValueListElement
    owner: SubmodelElementList
    domain_of:
    - SubmodelElementList
    range: AasSubmodelElements
    required: true
  value:
    name: value
    description: Submodel element contained in the list.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    alias: value
    owner: SubmodelElementList
    domain_of:
    - Blob
    - DataSpecificationIec61360
    - Extension
    - File
    - Key
    - MultiLanguageProperty
    - OperationVariable
    - Property
    - Qualifier
    - ReferenceElement
    - SpecificAssetId
    - SubmodelElementCollection
    - SubmodelElementList
    - ValueReferencePair
    range: SubmodelElement
    multivalued: true
  valueTypeListElement:
    name: valueTypeListElement
    description: The value type of the submodel element contained in the list.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: valueTypeListElement
    owner: SubmodelElementList
    domain_of:
    - SubmodelElementList
    range: DataTypeDefXsd
  category:
    name: category
    description: The category is a value that gives further meta information w.r.t.
      to the class of the element. It affects the expected existence of attributes
      and the applicability of constraints.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: category
    owner: SubmodelElementList
    domain_of:
    - Referable
    range: string
  checksum:
    name: checksum
    description: Checksum to be used to determine if an Referable (including its aggregated
      child elements) has changed.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: checksum
    owner: SubmodelElementList
    domain_of:
    - Referable
    range: string
  description:
    name: description
    description: Description or comments on the element.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: description
    owner: SubmodelElementList
    domain_of:
    - Referable
    range: LangString
    multivalued: true
  displayName:
    name: displayName
    description: Display name. Can be provided in several languages.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: displayName
    owner: SubmodelElementList
    domain_of:
    - Referable
    range: LangString
    multivalued: true
  idShort:
    name: idShort
    description: In case of identifiables this attribute is a short name of the element.
      In case of referable this ID is an identifying string of the element within
      its name space.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: idShort
    owner: SubmodelElementList
    domain_of:
    - Referable
    range: string
    pattern: ^[a-zA-Z][a-zA-Z0-9_]+$
  kind:
    name: kind
    description: 'Kind of the element: either type or instance.'
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: kind
    owner: SubmodelElementList
    domain_of:
    - HasKind
    - Qualifier
    range: ModelingKind
  semanticId:
    name: semanticId
    description: Identifier of the semantic definition of the element. It is called
      semantic ID of the element or also main semantic ID of the element.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: semanticId
    owner: SubmodelElementList
    domain_of:
    - HasSemantics
    range: Reference
  supplementalSemanticIds:
    name: supplementalSemanticIds
    description: Identifier of a supplemental semantic definition of the element.
      It is called supplemental semantic ID of the element.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: supplementalSemanticIds
    owner: SubmodelElementList
    domain_of:
    - HasSemantics
    range: Reference
    multivalued: true
  qualifiers:
    name: qualifiers
    description: Additional qualification of a qualifiable element.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: qualifiers
    owner: SubmodelElementList
    domain_of:
    - Qualifiable
    range: Qualifier
    multivalued: true
  embeddedDataSpecifications:
    name: embeddedDataSpecifications
    description: Embedded data specification.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: embeddedDataSpecifications
    owner: SubmodelElementList
    domain_of:
    - HasDataSpecification
    range: EmbeddedDataSpecification
    multivalued: true
  extensions:
    name: extensions
    description: An extension of the element.
    from_schema: https://admin-shell.io/aas/3/0/RC02
    rank: 1000
    alias: extensions
    owner: SubmodelElementList
    domain_of:
    - HasExtensions
    range: Extension
    multivalued: true

```
</details>